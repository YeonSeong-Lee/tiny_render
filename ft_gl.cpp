...
void triangle(Vec3i *pts, IShader &shader, TGAImage &image, TGAImage &zbuffer) {
  ...
  for (P[0] = bboxmin[0]; P[0] <= bboxmax[0]; P[0]++) {
    for (P[1] = bboxmin[1]; P[1] <= bboxmax[1]; P[1]++) {
      ...
      P[2] = std::max(0, std::min(255, int(pts[0].z * c[0] + pts[1].z * c[1] +
                                           pts[2].z * c[2] +
                                           .5)));  // clamping to 0-255 since it
                                                   // is stored in unsigned char
      if (c[0] < 0 || c[1] < 0 || c[2] < 0 || zbuffer.get(P[0], P[1])[0] > P[2])
        continue;
